...
import { User as UserIcon, Calendar, Mail, Shield, HardDrive, Camera, Trash2, Loader2, KeyRound } from "lucide-react"
...
function MyProfileDialog({ isOpen, onClose, user }: { isOpen: boolean; onClose: () => void; user: UserWithQuota }) {
	...
	const handleResetPassword = () => {
		forgotPassword.mutate({ email: user.email }, {
			onSuccess: () => {
				toast.success("Reset link generated in backend logs")
			},
			onError: (error: any) => {
