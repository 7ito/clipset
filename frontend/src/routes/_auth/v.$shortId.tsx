...
function VideoPlayerPage() {
  ...
  const handleTimeUpdate = useCallback((time: number) => {
    ...
    // Count view when 30% of video has been watched
    if (
      !hasIncrementedView && 
      video?.processing_status === "completed" &&
      video?.duration_seconds &&
      time >= video.duration_seconds * 0.3
    ) {
  ...
  }, [hasIncrementedView, video?.processing_status, video?.duration_seconds, shortId, queryClient])
